...
const SubmissionCard = ({data}) => {
    ...
    console.log({isStudent, userProfile});
    ...
    function giveMarks(formData) {
        setIsLoading(true);
        allotMarks(currentUser, params.classroomID, params.postID, data.owner, formData.points || 0, data?.id)
            .then((response) => {
                setIsLoading(false);
        ...
        <div className="card border-0 rounded-5 shadow-sm p-3">
            <div className="d-flex align-items-center">
                ...
                <div className="flex-fill ms-3">
                    <div className="lh-1 fw-bolder text-900">{data?.user?.name || "-"}</div>
                </div>
